...
export default function Navbar(props: any) {
  ...
  const signInOutButton = () => {
    if (status === "authenticated") {
      return (<button onClick={() => signOut({ callbackUrl: "/" })} className="block border-2 border-transparent bg-purple-600 rounded-lg hover:shadow-lg hover:shadow-indigo-600 bg-transparent p-2 hover:text-purple-600 hover:border-purple-600 hover:bg-purple-600 hover:bg-opacity-10 ease-in-out duration-150">Sign Out</button>);
    }
    return (<Link href="/?login=true" className="block border-2 border-transparent bg-purple-600 rounded-lg hover:shadow-lg hover:shadow-indigo-600 bg-transparent p-2 hover:text-purple-600 hover:border-purple-600 hover:bg-purple-600 hover:bg-opacity-10 ease-in-out duration-150">Login</Link>);
  }
      ...
      <div className={`${isNavbarExpanded ? '' : 'hidden'}`}>
        <div>
          <ul className="text-white font-bold text-xl text-center mt-5">
            ...
            <li>
              <Link href={navItems[0][0]} className="w-fit mx-auto mb-1 block border-2 border-transparent bg-purple-600 rounded-lg hover:shadow-lg hover:shadow-indigo-600 bg-transparent p-2 hover:text-purple-600 hover:border-purple-600 hover:bg-purple-600 hover:bg-opacity-10 ease-in-out duration-150">{navItems[0][1]}</Link>
            </li>
            <li>
              <Link href={navItems[1][0]} className="w-fit mx-auto mb-1 block border-2 border-transparent bg-purple-600 rounded-lg hover:shadow-lg hover:shadow-indigo-600 bg-transparent p-2 hover:text-purple-600 hover:border-purple-600 hover:bg-purple-600 hover:bg-opacity-10 ease-in-out duration-150">{navItems[1][1]}</Link>
            </li>
            <li>
              <Link href={navItems[2][0]} className="w-fit mx-auto mb-1 block border-2 border-transparent bg-purple-600 rounded-lg hover:shadow-lg hover:shadow-indigo-600 bg-transparent p-2 hover:text-purple-600 hover:border-purple-600 hover:bg-purple-600 hover:bg-opacity-10 ease-in-out duration-150">{navItems[2][1]}</Link>
            </li>
            <li>
              {signInOutButton()}
            </li>
